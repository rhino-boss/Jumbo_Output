...
**每區預設只顯示 3 筆**，超過就出現「全部顯示（總數）」按鈕，就地展開／收起；
總數 ≤ 3 時按鈕自動隱藏。搜尋時不套這個上限、按鈕也隱藏 —— 使用者已經
自己縮小範圍了，再砍成 3 筆只會擋路。

首頁的 Demogame **固定用遊戲卡**，沒有卡片／清單切換 —— 那個切換只在
「完整頁 →」（`games.html`）提供。
...
「完整頁 →」進 `games.html`：全部遊戲、搜尋，加三顆按鈕 ——

| 按鈕 | 行為 |
|---|---|
| `⇅ 更新時間` / `⇅ 代號` | 一顆按鈕，點一下切換排序依據 |
| `↓` / `↑` | 切換升冪／降冪 |
| `☰ 清單` / `▦ 卡片` | 切換檢視模式 |

三者都是「點一下就切換」的同一種操作邏輯。排序與檢視模式記在 `localStorage`。
