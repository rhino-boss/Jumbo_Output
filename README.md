...
分群標籤取自 `catalog.js` 的 `game` 欄，**每一列都會顯示**，沒填就標「其他」。
各群不同顏色（以 `data-tag` 對應 CSS）：
...
在 `catalog.js` 填了沒對應顏色的新標籤時會落到預設灰，不會壞掉 ——
要加顏色就在 `style.css` 補一條 `data-tag` 規則。標籤等寬 46px，
所以各列的名稱會對齊成一直排。

> 這一區**不套**「標題開頭已含該標籤就不重複顯示」那條規則 ——
> 這裡的標籤是用來分群配色的，每列都要有；否則「HR 系統」、
> 「測試環境 — Club」這種標題開頭剛好等於標籤的就會被藏起來。
