...
| 分析報告 | 紅 `#d9534f` | 自動掃描 `Project/競品分析/遊戲數據_*.html` | 卡片格 | 4 |
...
報告類的卡片（分析報告、其他報告）底部顯示「更新 YYYY-MM-DD」：
分析報告取競品分析 `README.md` 的「報告日期」欄，其他報告取 `catalog.js` 的 `date`。
...
「H016 幸運王牌」，代號會重複所以自動隱藏；分析報告的廠商不在標題裡，照樣顯示。
